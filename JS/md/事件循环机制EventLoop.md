...
**console.log(111)** 永远都不会输出, 因为 **javaScript** 是单线程, 而Event Loop即事件循环，是解决javaScript单线程运行阻塞的一种机制
...
主线程从 **任务队列** 中读取事件,主线程不断重复上面的第三步 这个过程是循环不断的，所以整个的这种运行机制又称为 Event Loop（事件循环）
